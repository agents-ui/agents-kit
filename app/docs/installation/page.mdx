...
Original Beautiful UI entries use the `beautiful-original-` prefix. Import their copied `components/beautiful-ui/original/beautiful-ui.css` once and wrap them in an ancestor with `className="beautiful-ui-scope"`. The host's `.dark` class selects the dark palette. The earlier `beautiful-` adapters remain available at their existing paths.
...
The [source coverage report](https://github.com/agents-ui/agents-kit/blob/main/docs/source-coverage.md) lists the imported catalogs, preserved variants, and remote demo-media exceptions.
